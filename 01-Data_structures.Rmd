...
## Vectors
<!-- Ich finde das html-markup echt untschön zu lesen in markdown. Da muss es doch eine alternative geben, oder? -->
<!-- Im Zweifel würde ich wohl eher auf die Farbe verzichten, als die Quelldatei so unübersichtlicher zu machen. -->
1. __<span style="color:red">Q</span>__: What are the six types of atomic vector? How does a list differ from an
   atomic vector?  
<!-- Frage vielleicht kursiv setzen und Antwort normal? Sorgt auch für Gliederung und Übersicht. -->
__<span style="color:green">A</span>__: The six types are logical, integer, double, character, complex and raw. The elements of a list don't have to be of the same type.
...
<!-- "is behind", das formuliert man doch anders, wie nochmal? (hab gerade kein Inet... :) -->

6. __<span style="color:red">Q</span>__: Why is the default missing value, `NA`, a logical vector? What's special
   about logical vectors? (Hint: think about `c(FALSE, NA_character_)`.)  
__<span style="color:green">A</span>__: It is a practical concern. When you combine `NA`'s in `c()` with other atomic types they will be coerced like `TRUE` and `FALSE` to integer `(NA_integer_)`, double `(NA_real_)`, complex `(NA_complex_)` and character `(NA_character_)`.

## Attributes

1.  __<span style="color:red">Q</span>__: An early draft used this code to illustrate `structure()`:

<!-- Warum indentest du hier den Code im R-Cunk? -->
    ```{r}
...
3.  __<span style="color:red">Q</span>__: How would you describe the following three objects? What makes them
    ...
    ```  
<!-- Kleines "Style-Guide"-update für die comments above:) -->
...
<!-- Bin unsicher mit dem Komma nach "Otherwise" -->
