...
1. __[Q]{.Q}__: What would a full `setOldClass()` definition look like for an ordered factor (i.e. add `slots` and `prototype` to the definition above)?
   ...
   __[A]{.solved}purpose of `setOldClass()` lies in registering an S3 class as a "formally defined class", so that it can be used within the S4 OOP-system. When using it, we can also provide the argument`S4Class`, which will inherit the slots and the default values (prototype) to the registered class.
