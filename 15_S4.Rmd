...
1. __[Q]{.Q}__: `lubridate::period()` returns an S4 class. What slots does it have? What class is each slot? What accessors does it provide?
   ...
   __[A]{.solved}__: Objects of the S4 `Period` class have six slots named `.Data`, `year`, `month`, `day`, `hour` and `minute`, which are each of type double. Most fields can be retrieved by an identically named accessor (e.g. `lubridate::year()` will return the field). Just the `.Data` slot has a specially named accessor, called `methods::gedDataPart()`.
...
2. __[Q]{.Q}__: What happens if you define a new S4 class that doesn’t have any slots?  (Hint: read about virtual classes in `?setClass`.)
   ...
   To get around that, we register the old class before we define the new one.
...
1. __[Q]{.Q}__: What would a full `setOldClass()` definition look like for an ordered factor (i.e. add `slots` and `prototype` to the definition above)?

   __[A]<!-- @Hadley: New solution, please review. -->
