...
1. __[Q]{.Q}__: What would a full `setOldClass()` definition look like for an ordered factor (i.e. add `slots` and `prototype` the definition above)?
   ...
   __[A]{.open}started
   <!-- @Hadley: New solution, please review. -->

   Thoughts for an introduction
   - What does setOldClass() do? it registers an S4 class as an S3 class, so that it can be used with S3 methods. (correct?)


   From `?setOldClass`
   - registers it as an S3 class  vs Register an old-style (a.k.a. ‘S3’) class as a formally defined class.

   - `S4Class`: optionally, the class definition or the class name of an S4 class. The new class will have all the slots and other properties of this class, plus any S3 inheritance implied by multiple names in the Classes argument. See the section on “S3 classes with known attributes” below.
   

   From `?Methods_for_S3`
   - The systems can also be combined by using an S3 class with S4 method dispatch or in S4 class definitions.
   ...
   From Advanced R
   Calls to setOldClass() in the code for a package allow the class to be used as a slot in formal (S4) classes and in signatures for methods (see Methods_for_S3).
   
   In slots and contains you can use S4 classes, S3 classes, or the implicit class 
   ...
   - build an S4 class on top of an S3 class   
   ...
   Template from book (setOldClass seems to be raising an error)
    ```{r, error=TRUE}
    setClass(Class = "factor",
             contains = "integer",
             slots = c(
               levels = "character"
             ),
   ...
   First Attempt at solution
    ...
    OrderedS4 <- setClass("ordered",
                          contains = "factor",
                          slots = c(
                            levels = "character"
                          ),
                          prototype = structure(
                            factor(),  # ordered() | factor(ordered = TRUE) ??
                            levels = character()
                          )
    )
    
    
    a <- OrderedS4(1L, levels = "A")
    attributes(a)
    a@.Data
    ```

   does the S3 dispatch / inheritance work?
