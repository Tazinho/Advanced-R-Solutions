...
1. __<span style="color:red">Q</span>__:  To practice your function writing skills, convert the following functions into C++. For now, assume the inputs have no missing values.  
   ...
   __<span style="color:orange">A</span>__: (TODO: The last one, `var()` still needs to be implemented)
...
1. __<span style="color:red">Q</span>__:  Rewrite any of the functions from the first exercise to deal with missing values. If `na.rm` is true, ignore the missing values. If `na.rm` is false, return a missing value if the input contains any missing values. Some good functions to practice with are `min()`, `max()`, `range()`, `mean()`, and `var()`.

   __<span style="color:orange">A</span>__: (TODO: `var`'s implementation is still missing)
...
2. __<span style="color:red">Q</span>__: Rewrite `cumsum()` and `diff()` so they can handle missing values. Note that these functions have slightly more complicated behaviour.  
...
1. __<span style="color:red">Q</span>__: `median.default()` using `partial_sort`.
...
2. __<span style="color:red">Q</span>__: `%in%` using `unordered_set` and the `find()` or `count()` methods.

   __<span style="color:green">A</span>__:

3. __<span style="color:red">Q</span>__: `unique()` using an `unordered_set` (challenge: do it in one line!).  

   __<span style="color:orange">A</span>__: (TODO: address the challenge.)
...
4. __<span style="color:red">Q</span>__: `min()` using `std::min()`, or `max()` using `std::max()`.
...
5. __<span style="color:red">Q</span>__: `which.min()` using `min_element`, or `which.max()` using `max_element`.  
...
6. __<span style="color:red">Q</span>__: `setdiff()`, `union()`, and `intersect()` for integers using sorted ranges and `set_union`, `set_intersection` and `set_difference`.
