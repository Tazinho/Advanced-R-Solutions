...
1. __[Q]{.Q}__:  Rewrite any of the functions from the first exercise to deal with missing values. If `na.rm` is true, ignore the missing values. If `na.rm` is false, return a missing value if the input contains any missing values. Some good functions to practice with are `min()`, `max()`, `range()`, `mean()`, and `var()`.
   ...
   __[A]{.solved}__: Before we take a look at `cumsum()`, `any()`, `Position()` and `pmin()` from the first exercise, we follow the advice and rewrite `min()`, `max()`, `range()`, `mean()` and `var()`. When ignoring missing values (i.e. `na_rm = false`), we try to keep the overall function behaviour close to the original function. To avoid irregular type conversions we mostly stick with vector data types as return values.
...
2. __[Q]{.Q}__: Rewrite `cumsum()` and `diff()` so they can handle missing values. Note that these functions have slightly more complicated behaviour.
   ...
   __[A]{.solved}__: As we already wrote an `NA`-aware `cumsumC()` function under the assumption to return a single `NA` in case of `na_rm = false`, we modify it here a bit to always return a vector with the same length as it's `x` argument. Our new `cumsumC2()` treats `NA`'s always like zeros. In case of `na_rm = false` `NA`s are kept in the output and in case of `na_rm = true` `NA`s are replaced by the last occuring non-`NA`-value or zero, in case of only `NA`s. 
   ...
   For `diffC()`'s implementation, we stick again to the idiom from the last exercise and return just an `NA` whenever one occures. In case of `na_rm = true`, we ensure that calculations which are affected by `NA`'s will also return an `NA`. (An alternative would be to exclude `NA`s completely from the input or - equivalently - the output).
