...
1. __[Q]{.Q}__: Weigh the pros and cons of `download.file %>% dot_every(10) %>% delay_by(0.1)` vs `download.file %>% delay_by(0.1) %>% dot_every(10)`.
   ...
   __[A]{.solved}__: `download.file %>% dot_every(10) %>% delay_by(0.1)` means that you print the dot per every 10 downloads and then delay by 0.1. Where `download.file %>% delay_by(0.1) %>% dot_every(10)` means that you delay by 0.1 every downloads, and then dot per every 10 downloads. So there is no difference in that you print the dot for every 10 downloads. But there is a difference in that you delay for 0.1 for every 10 downloads, or, you delay for 0.1 for each downloads.
   <!-- It seems quite obvious, but maybe there are some reasons that you didn't write this out..? Please check this out -->
