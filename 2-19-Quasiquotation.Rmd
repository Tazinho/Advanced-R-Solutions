...
2. __[Q]{.Q}__: The following two calls print the same, but are actually different:
   ...
   __[A]{.solved}__: We quote expressions with `expr()`  evaluate them with `!!()`. In the expression assigned to `b` the call `1:10` is evaluated to an integer vector, while still being a call object in `a`.
...
1. __[Q]{.Q}__: In the linear-model example, we could replace the `expr()` in `reduce(summands, ~ expr(!!.x + !!.y))` with `call2()`: `reduce(summands, call2, "+")`. Compare and contrast the two approaches. Which do you think is easier to read?
   ...
   __[A]{.solved}__: We would consider the first version to be more readable. There seems to be a little more boilerplate code at first, but the unquoting syntax is very readable. Overall whole expression seems more more explicit and less complex.
