...
1. __<span style="color:red">Q</span>__: For each function in the following base R code, identify which arguments are quoted and which are evaluated.
   ...
   __<span style="color:green">A</span>__: For each argument we follow the advice from the textbook and execute it outside of the function first.
   
   As `MASS` raises an error when executed alone, we can clearly see that `library()` quotes it''s first argument.
   ...
   The same holds for the second arguments in `subset()` and `with()`:
    ...
    sum(vs)                 # sum(vs) -> quoted
    ```

   In the following code, the `$()` function clearly quotes `am`:
   ...
   The last example is a bit harder to explain. In short: 

    ```{r}
    rm(mtcars2)  # mtcars2 -> quoted
    ```
   
   When we inspect the source code of `rm()`, we notice that `rm()` catches it's `...` argument internally via `match.call()` as an unevaluated call (in this case a pairlist) and afterwards converts it into a string for further evaluation.
