...
1.  __<span style="color:red">Q</span>__: Why isn't `is.na()` a predicate function? What base R function is closest
    ...
    __<span style="color:green">A</span>__: Because a predicate function always returns `TRUE` or `FALSE`. `is.na(NULL)` returns `logical(0)`, which excludes it from being a predicate function. The closest in base that I am aware of is `anyNA()`, if one applies it elementwise.
...
2.  __<span style="color:red">Q</span>__: Create a table that has _and_, _or_, _add_, _multiply_, _smaller_, and 
    ...
    Notice that we were relatively strict about the _binary_ row. Since the _vectorised_ versions are more general, then the _binary_ versions, we could have used them twice. However, this doesn't seem to be the intention of this exercise. 
    
    The last part of this exercise can be solved via copy pasting from the book and the last exercise for the _binary_ row and creating combinations of `apply()` and the _reducing_ versions for the _array_ row. I think the array functions just need a dimension and an `rm.na` argument. I don't know how I would name them, but sth. like `sum_array(1, na.rm = TRUE)` could be ok.
    ...
    The other point are the naming conventions. I think they are clear, but it could be useful to provide the missing binary operators and name them for example `++`, `**`, `<>`, `><` to be consistent.
