...
2.  __<span style="color:red">Q</span>__: Why is using `sapply()` to get the `class()` of each element in 
    ...
    __<span style="color:green">A</span>__: Columns of data.frames might have more than one class, so the class of `sapply()`'s output may (silently) differ from time to time. If ...
