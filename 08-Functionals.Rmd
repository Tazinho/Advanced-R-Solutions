...
3.  __<span style="color:red">Q</span>__: Use both for loops and `lapply()` to fit linear models to the
    ...
    # for loop
    lf <- vector("list", length(formulas))
    for (i in seq_along(formulas)){
      lf[[i]] <- lm(formulas[[i]], data = mtcars)
    }
    ```
    
    Note that all versions return the same content, but the objects will differ in the values of the "call" element.
