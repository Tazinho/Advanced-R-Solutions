...
- Each chapter of this book was rendered in a separate R session via the `{bookdown}` package. We configured this process to initially
  - set `` `%>%` <- magrittr::`%>%` `` to unlock the pipe operator without specifically loading `{magrittr}` every time,
...
- Benchmark are computed when the book is rendered. This improves reproducibility, but the exact results depend on the system creating the document. It may also lead to minor differences between the print and online version.
