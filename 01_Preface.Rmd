...
Because this book builds so heavily on Advanced R, we think it should be read together with the textbook, either as a hardcopy or in the online version (https://adv-r.hadley.nz). Working on the exercises by yourself first generally should give you the biggest benefit.
...
- We chose to keep the code in this book as self-contained as possible. The packages used will be loaded in the beginning of each chapter. By default, we will include all code necessary for the examples. When some longer code passages (from the textbook) were omitted this will be stated explicitly.
