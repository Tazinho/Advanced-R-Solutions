...
This book provides worked-out solutions to the exercises given in Advanced R (2nd Edition, Wickham, 2019) and reflects our efforts to understand and acquire the content presented in it.
...
When we came across Advanced R, we found that it gave us a more thorough understanding of the R code we worked with daily and helped us to see the underlying principles more clearly. The content helped us to get a more complete picture of the landscape of programming with R.
...
We repeated this process until each exercise was reviewed and approved. As a result, we feel pretty good about the quality of the solutions in the book. However, any remaining mistakes or inconsistencies are certainly on us.

## How to use this book

Since this book builds so heavily on Advanced R, we think it should be read together with the textbook, either as a hardcopy or the online version (https://adv-r.hadley.nz). Working on the exercises by yourself first should give you the biggest benefit.
...
Of the more difficult exercises in the book, only a few were completed in one go. Often we had to reread the question, look up the related content in Advanced R and started by writing a few first lines of code or consulted the documentation. Reading the source code (preferably with syntax highlighting) and searching the web were typically also quite helpful.

To support your study, you may also be interested in the [R4DS Advanced R Bookclub](https://GitHub.com/r4ds/bookclub-Advanced_R), where cohorts of readers dicuss a different chapter of the textbook every week.

In case you want to do more or have a special interest in the mechanics of base R, you may also be interested in checking out the first edition of Advanced R (http://adv-r.had.co.nz/). Some additional solutions related to that edition can be found at https://advanced-r-solutions-ed1.netlify.app/
...
Any open source project is actually the result of the work of a lot of people. We would like to explicitly mention and thank everybody who contributed solutions, raised questions, or helped to fix spelling and grammar.
...
- `{bookdown}` will render each chapter in a separate R session. Initially each chapter will set ``%>%` <- magrittr::`%>%`` to unlock the pipe operator (without loading `{magrittr}`), set a random seed (`1014` as in Advanced R) to improve reproducibility and define a few `{ggplot2}` and `{knitr}` options. You can check out the (exact code)[https://GitHub.com/Tazinho/Advanced-R-Solutions/blob/main/common.R] on GitHub.
- We chose to keep the code in this book as self-contained as possible. The packages used will be loaded in the beginning of each chapter. By default, we will include all code necessary for the examples. When some longer code passages (from Advanced R) were omitted this will be stated explicitly.
