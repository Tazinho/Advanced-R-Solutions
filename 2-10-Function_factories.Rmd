...
1. __<span style="color:red">Q</span>__: The definition of `force()` is simple:
   ...
   __<span style="color:green">A</span>__: As you can see `force(x)` is just syntactic sugar for `x`. From the textbook:
...
6. __<span style="color:red">Q</span>__: What happens if you use `<-` instead of `<<-`? Make predictions, then verify with the code below.
   ...
   __<span style="color:green">A</span>__: Without the superassignment `<<-`, the counter will always return 1. The counter always starts in a new execution environment within the same enclosing environment, which contains an unchanged value for `i` (in this case it remains 0).
