...
As expected we see that idiomatic primitive function `sqrt()` is the fastest. The approach `exp(log(x) / 2)` which builds up on two primitive functions is second, even though already considerably slower. The other calculations are even slower: `x ^ 0.5` is (slightly) faster than `x ^ (1 / 2)`, because `0.5` requires less computation than `(1 / 2)` and everything stays the same.
