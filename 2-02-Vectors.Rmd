...
5. __<span style="color:red">Q</span>__: Precisely what do is.atomic(), is.numeric(), and is.vector() test for?
   ...
   __<span style="color:green">A</span>__: __`is.atomic()`__ tests if `typeof()` is one of `"logical"`, `"integer"`, `"double"`, `"complex"`, `"character"`, `"raw"` or `"NULL"`. __`is.numeric()`__ tests if `typeof()` is integer or double and class is not one of `"factor"`, `"Date"`, `"POSIXt"` or `"difftime"`. __`is.vector()`__ tests if an object has no attributes, except of names and if its `mode()` is atomic (`"logical"`, `"integer"`, `"double"`, `"complex"`, `"character"`, `"raw"`), `"list"` or `"expression"`.
