...
```{r}
# 3 different classes
date    <- as.Date("1970-06-01")
dttm_ct <- as.POSIXct("1970-06-01 22:00", tz = "UTC")
dttm_lt <- as.POSIXlt(dttm_ct)

# combining date and date-time
c(date, dttm_ct)  # strange!
```

it is first converted into the difference in seconds (2757600) and then this is converted into date. Therefore the conversion will be 1970-01-01 + 2757600 days. -> Method dispatch happens on the first argument. To see explicitly what is happening, we can call the methods directly:

```{r}
# reason:
unclass(c(date, dttm_ct))
c.Date(dttm_ct,    date)  # distorts dttm_ct
c.POSIXct(dttm_ct, date)  # distorts date
c.POSIXlt(dttm_lt, date)  # o.k.
```
...
unlist(list(date, dttm_lt))
```

- Date vectors are internally integers so the problem really is that attributes are stripped. 
- In essence, as long as it's a vector, unlist does nothing (no drop of attributes as well). And if it's a list, then attributes can't be preserved.


`c()` coerces types and errors may occur because of inappropriate method dispatch. `unlist()` strips attributes
