...
2. __<span style="color:red">Q</span>__: What happens if you attempt to set rownames that are not unique?
    ...
    __<span style="color:green">A</span>__
    It depends on how you attempt this. If you use the `row.names<-` replacement function, no further arguments are provided and the underlying `row.names<-.data.frame` method will throw an error (and an additional warning):
   
    ```{r}
    row.names(mtcars) <- rep(1, nrow(mtcars))
    ```
    
    However, by calling the underlying `.rowNamesDF<-` you can set the `make.names` argument to `NA`, which sets `seq_len(nrow(x))` as (integer) row names or to `TRUE`, which triggers a call of `make.names(value, unique = TRUE)` for constructing valid (character) row names.
...
3. __<span style="color:red">Q</span>__: If `df` is a data frame, what can you say about `t(df)`, and `t(t(df))`? Perform some experiments, making sure to try different column types.
    ...
    __<span style="color:green">A</span>__ Both will return matrices with dimensions regaring the typical transposition rules. As `t()` uses `as.matrix.data.frame()` for preprocessingg infront of applying `t.default()` and elements of matrices need to be of the same type, all elements get coerced in the usual order (logical << integer << double << character), while factors, dates and datetimes are treated as characters during coercion.
