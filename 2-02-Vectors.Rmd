...
1. __<span style="color:red">Q</span>__: How is setNames() implemented? How is unname() implemented? Read the source code.
   ...
   As the data comes first `setNames()` also well with the magrittr-pipe operator. When no first argument is given, the result is a named vector:
   
   ```{r}
   setNames( , c("a", "b", "c"))
   ```
   
   However, the implemention also means that `setNames()` only effects the names attribute and not any ohter more specific naming-attributes like the dimnames attribute for matrices and arrays.
   ...
   It always removes names and dimnames attributes when they are set, except for data frames. Note that the documentation is currently not correct (R 3.5.1). `force == TRUE` doesn't remove the dimnames (names and row names) as the underlying `dimnames<-.data.frame` doesn't handle `NULL` and will always catch it within its first error condition:
...
2. __<span style="color:red">Q</span>__: What does dim() return when applied to a 1d vector? When might you use NROW() or NCOL()?
   ...
   (TODO: The following answer originally answers the question:
   When might you use `NROW()` or `NCOL()`? Therefore, the answer has to change according to the new question (maybe one of the old exercises at the bottom of this chapter contains info that can be merged))
    
   __<span style="color:orange">A</span>__: From `?nrow`:
   ...
   This behaviour might be useful, to guarantee a numeric result even in cases one is not sure if the result is an atomic or a data frame, i. e. when subsetting of a data frame without `drop = TRUE` is involved:
    
    ```{r}
    nrow(iris[ , "Sepal.Width"])
    ncol(iris[ , "Sepal.Width"])
    NROW(iris[ , "Sepal.Width"])
    NCOL(iris[ , "Sepal.Width"])
    ```
