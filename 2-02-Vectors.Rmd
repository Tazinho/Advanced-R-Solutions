...
1. __<span style="color:red">Q</span>__: What sort of object does `table()` return? What is its type? What attributes does it have? How does the dimensionality change as you tabulate more variables?
   ...
   __<span style="color:green">A</span>__: `table()` returns a contingency table of its input variables, which has the class `"table"`. Internally it´s represented as an array (implicit class) of integers (type) with the attributes `dim` (dimension of the underlying array) and `dimnames` (one name for each input column). The dimensions correspond to the number of unique values in each input variable.
...
1. __<span style="color:red">Q</span>__: List all the ways that a list differs from an atomic vector.
   ...
   - Atomic vectors point to only one address in memory, while lists contain a separate references for each element
...
2. __<span style="color:red">Q</span>__: Why do you need to use `unlist()` to convert a list to an atomic vector? Why doesn't `as.vector()` work?  
   ...
   <!-- old: -->
   <!-- To strip (flatten) the nested structure `as.vector()` doesn't work, because a list is already a vector. -->
...
<!-- @malte: recheck this for clarity and correctness. -->
