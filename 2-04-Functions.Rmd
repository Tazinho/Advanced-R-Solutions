...
1.  __<span style="color:red">Q</span>__: What important property of `&&` make `x_ok()` work?
    ...
    __<span style="color:green">A</span>__: The intended usage of `x_ok` is to check if an argument provided to a function is not NULL, has length 1 and is greater then 0. To work with this function, we only want to know if this is `TRUE`, `FALSE` or NA (unknown). Therefore the first version behaves as desired as we can see in the first and the third example.
    
    `&&` works because it checks from left to right, if **the first element** of every side of the operator evaluates to `TRUE`, `FALSE` or sth. else (another valid value, which gets coerced to logical). In case of `FALSE`, it stops and returns `FALSE`. If none is `FALSE` but at least one is not `TRUE` (after checking all sides), `NA` is returned. This makes sense, since if sth. is neither true nor false, it's logical value is unknown, corresponding to `NA`.
    
    Apart from the undesired elementwise evaluation of the `&` operator (at least in this case) it almost works the same. From `?`??``:
