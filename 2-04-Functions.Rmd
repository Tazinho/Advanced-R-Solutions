...
2. __<span style="color:red">Q</span>__: It’s possible (although typically not useful) to call an anonymous function. Which of the two approaches below is correct? Why?
   ...
   __<span style="color:green">A</span>__: The second approach is correct. In this approach the anonymous function `function(x) 3` is surrounded by a pair of brackets and followd by `()` which calls the former `(function(x) 3)`. The extra syntax is needed as otherwise the following `()` belong to the body of the anonymous function. Therefore, the expression in the first approach returns a function with the invalid body `3()` and we get a error when we call it:
