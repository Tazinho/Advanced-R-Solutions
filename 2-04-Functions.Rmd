...
3. __<span style="color:red">Q</span>__: What does this function return? Why? Which principle does it illustrate?
   ...
   __<span style="color:orange">A</span>__: The function returns `c(2, 1)`. This is due to name masking. When `x` gets accessed within `c()`, the promise `x = {y <- 1; 2}` gets evaluated inside `f1()`'s environment. `y` gets the the value `1` assigned and the return value of `{()` (`2`) is assigned to `x`. When `y` gets accessed within `c()`, it has already the value `1` and R doesn't need to look it up any further. Therefore, the promise `y = 0` doesn't get evaluated. 
   
   As `y` got assigned during the evaluation of a default argument, the value of the global variable `y` stays untouched.
