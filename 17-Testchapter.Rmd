...
1.  __<span style="color:red">Q</span>__: Use `ast()` and experimentation to figure out the three arguments to an `if()` call. What would you call them? Which components are required?
    ...
    __<span style="color:green">A</span>__: You can write an `if()` statement in several versions: with or without else, formatted or in one line and also in infix notation. Here are several versions focussing on the possibility of leaving out curly brackets.
...
1.  __<span style="color:red">Q</span>__: What are the arguments to the `for()` and `while()` calls? 
    ...
    `while` requires a condition and an expression. Again an example in infix notation:
...
1.  __<span style="color:red">Q</span>__: What does the call tree of an `if` statement with multiple `else if` 
