...
1.  __<span style="color:red">Q</span>__: What does the following code return? What base type is built on?
    ...
    __<span style="color:green">A</span>__: It returns the Empirical Cumulative Distribution Function of `rpois(100, 10)`,
so it has the class "ecdf". Since this is a function and one usually calculates it
from a finite sample, it is/inherits also from the classes "stepfun" and "function".
    
    It is built on the base type "closure" and it saves the expression, which was used to create it, (as a language object) in its `call` attribute.
