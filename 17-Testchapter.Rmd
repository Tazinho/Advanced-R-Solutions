...
1.  The following code accesses the mean function in multiple different ways.
    ...
    __<span style="color:green">A</span>__: Yes, they point to the same object. To confirm that we look at the address of the underlying function object. Since `lobstr::obj_addr()` currently returns an error, we assign each option to a new name and inspect the address via `pryr::address()`:
...
1.  What rules does `make.names()` use to convert non-syntactic names into
    ...
    __<span style="color:green">A</span>__: A valid name starts with a letter or a dot (not followed by a number). It also consists only of letter, numbers, dots and underscores (since R version 1.9.0). There three main strategies applied to construct syntactically valid names (see also `?make.names`):
    ...
    * reserved R keywords (see `?reserved`) get prepended by a dot:
...
1.  I slightly simplified the rules that govern syntactic names. Why is `.123e1`
    ...
    __<span style="color:green">A</span>__: It is not syntactically valid as it starts with one dot and is followed by a number.
