...
1.  Predict the output of the following code:
    ...
    Since `x` and `y` names with bindings to objects that point to the same reference, no additional memory is needed and our prediction is the maximum memory of both objects (y; 4000040 B):
    
    ```{r}
    unclass(pryr::object_size(x, y))
    ```
    
    The next one gets a bit more tricky. Since the first element of `y` becomes different to `x`, a completely new object is created in memory. Hence 10 is of type double, the new object will take more memory. A double needs 40 B + length * 8 B (overall 8000040 B). So we get: first element of `y` (8000040 B) + second element of `y` (`x`; 4000040 B) + list of length 2 (40 B + 16 B) = 12000136 B as our prediction:
    ...
    Again all elements of `x` are shared within `y` (`x` is the second element of `y`). So there overall memory usages corresponds to `y``s:
    ...
    Now `x` and `y` don't share any values anymore (from Rs perspective) and their memory add up:
