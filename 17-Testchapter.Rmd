...
1.  __<span style="color:red">Q</span>__: What happens if you define a new S4 class that doesn't "contain" an existing class?  (Hint: read about virtual classes in `?setClass`.)
    ...
    __<span style="color:orange">A</span>__: It depends on the other arguments.
...
1.  __<span style="color:red">Q</span>__: In the defintion of the generic, why is it necessary to repeat the name of the generic twice?
    ...
    __<span style="color:orange">A</span>__: The first time it is needed as name of the generic and the second time it is needed to explicitly incorporate method dispacth via `standardGeneric()` within the generic's body (`def` parameter). This is similar to `UseMethod()` within S3.
...
1.  __<span style="color:red">Q</span>__: What's the difference between the generics generated by these two calls?
    ...
    __<span style="color:orange">A</span>__: The first call defines a standard generic and the second one creates a nonstandard generic. One can confirm this directly whlie printing (showing in S4 jargon) the function.
...
1.  __<span style="color:red">Q</span>__: What happens if you define a method with different argument names to the generic?
    ...
    __<span style="color:orange">A</span>__: It depends. Lets first create the object `hadley` of class "Person":
...
1.  __<span style="color:red">Q</span>__: What other ways can you find help for a method? Read `?"?"` and summarise the details.
    ...
    * general documentation of the methods from a specific generic via `methods?genericName`
    * documentation of a specific method via `ClassName?methodName`
    
    Regarding the latter, we can also get help on a special method via adding a `?` in front of a function call, like `?show(hadley)`.
...
1.  __<span style="color:red">Q</span>__: Take the last example which shows multiple dispatch over two classes that use multiple inheritance. What happens if you define a method for all terminal classes? Why does method dispatch not save us much work here?
    ...
    __<span style="color:orange">A</span>__: because we introduce ambiguity for all the 5 other classcombinations. One has distance 2 to all terminal nodes and the other 4 have distance 1 to two terminal nodes each.
