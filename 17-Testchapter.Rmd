...
1.  __<span style="color:red">Q</span>__: Use `ast()` and experimentation to figure out the three arguments to an `if()` call. What would you call them? Which components are required?
    ...
    __<span style="color:green">A</span>__: You can write an `if()` statement in several versions: with or without else, formatted or in one line and also in prefix notation. Here are several versions focussing on the possibility of leaving out curly brackets.
    ...
    One possible way of naming the arguments is 1. condition, 2. conclusion, 3. alternative.
...
1.  __<span style="color:red">Q</span>__: Two arithmetic operators can be used in both prefix and infix style.
    ...
    Of course also this doesn't make too much sense, since in `?Syntax` one can read, that R clearly differntiates between unary and binary `+` and `-` operators. (The former may not be used as an infix operator).

## R's grammar

1.  __<span style="color:red">Q</span>__: R uses parentheses in two slightly different ways as illustrated by 
    this simple call: `f((1))`. Compare and contrast the two uses. 
    
    __<span style="color:green">A</span>__: `(` is a primitive function and at the same time `(` is part of R's general prefix function syntax.
    
    So in the first example, we will see only the outer `(`, which  belongs to `f()` and is not shown in the Syntax, while the inner `(` is treated as a function (symbol) 
    ...
    For clearness, lets also create a third example, where none of the `(` is part of another functions syntax
    
    ```{r}
    lobstr::ast(((1 + 1)))
    ```
    
1.  __<span style="color:red">Q</span>__: `=` can also be used in two ways. Construct a simple example that shows 
    both uses.
...
1.  __<span style="color:red">Q</span>__: Compare `x + y %+% z` to `x ^ y %+% z`. What does that tell you about 
...
1.  __<span style="color:red">Q</span>__: `base::alist()` is useful for creating pairlists to be used for function
    arguments:
...
1.  __<span style="color:red">Q</span>__: `logical_abbr()` works with expressions. It currently fails when you give it
...
1.  __<span style="color:red">Q</span>__: Write a function that extracts all calls to a function.

1.  __<span style="color:red">Q</span>__: Instead of writing a wrapper that quotes its input, we could have made 
