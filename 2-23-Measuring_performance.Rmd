...
2. __<span style="color:red">Q</span>__: Here are two other ways to compute the square root of a vector. Which do you think will be fastest? Which will be slowest? Use microbenchmarking to test your answers.
   ...
   __<span style="color:orange">A</span>__: (TODO: Explain why the order behaves like this.) We'll use the "bench"-package to estimate the relative execution time of these expression, with the fastest expression standardized to 1.
...
<!-- todo: find better explanation, why `exp(log(x)/2)` is fast -->
<!-- also: explain, why (3) is faster than (2). I suppose it has sth todo with machine numbers and precision, maybe? -->
