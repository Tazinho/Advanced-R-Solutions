...
Tip: In case your dataset is stored in a database, you might want to check out the [`{modeldb}` package](https://github.com/tidymodels/modeldb) which executes the linear model code in the corresponding database backend.
...
bench::mark(
  ...
  RcppRoll = RcppRoll::roll_mean(x, n = 2, fill = NA, 
                                 align = "right"),
  slicer = slider::slide_dbl(x, mean, .before = 1, .complete = TRUE),
  TTR = TTR::SMA(x, 2),
...
- `{DEoptim}` provides a global optimiser based on the Differential Evolution algorithm.
