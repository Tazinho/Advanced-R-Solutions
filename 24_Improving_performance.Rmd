...
3. __[Q]{.Q}__: List four functions (not just those in base R) that convert a string into a date time object. What are their strengths and weaknesses?
   ...
   On the other hand, specifying the format and using `strptime()` can help to improve the character conversion rate.
    ...
    bench::mark(
      ...
      as.POSIXct_format = as.POSIXct("2020-01-01 12:30:25", format = "%Y-%m-%d %H:%M:%S"),
      strptime_fomat = strptime("2020-01-01 12:30:25", format = "%Y-%m-%d %H:%M:%S")
    )
    ```
    
   A fourth way is to use the methods from the lubridate package. Those converter functions are basically wrappers for the POSIXct approach and come with a more intuitive syntax, which allows to specify the format components as part of the converter function names.
   ...
   For further ways to convert characters into date time objects, have a look into the chron, the anytime and the fasttime packages. The chron package has it's own classes and stores times as fractions of days in the underlying double type. However, in contrast to the POSIXct and lt classes, chron doesn't deal with timezones and daylight savings. The anytime converts (accoring ot it's description) "Anything to POSIXct or Date". Similarly, the fasttime package's only function `fastPOSIXct()` is named after what the package does best.
