...
1. __[Q]{.Q}__: What are faster alternatives to `lm`? Which are specifically designed to work with larger datasets?
   ...
   __[A]{.solved}__: In the [Cran Task View for High Performance Computing](https://cran.rstudio.com/web/views/HighPerformanceComputing.html) we can find for example the `speedglm` package and its `speedlm()` function. For small datasets, there may be only minor performance gains (or even a small cost):

    ```{r}
    stopifnot(all.equal(
      coef(speedglm::speedlm(body_mass_g ~ bill_length_mm + species,
                             data = penguins)),
      coef(lm(body_mass_g ~ bill_length_mm + species,
              data = penguins))))
    
    bench::mark(
      "speedglm" = speedglm::speedlm(body_mass_g ~ bill_length_mm + species,
                                     data = penguins),
      "lm" = lm(body_mass_g ~ bill_length_mm + species,
                data = penguins),
      check = FALSE
    ) %>% 
   ...
   However on bigger datasets it can make a difference:
    ...
    bench::mark(
      "speedglm" = speedglm::speedlm(y ~ x1 + x2, data = td),
      "lm" = lm(y ~ x1 + x2, data = td),
      check = FALSE
    ) %>% 
      print_bench_results(expression, median)
    ```
    
   For further speed improvements, you might consider switching your linear algebra libraries (compare `?speedglm::speedlm`)
   ...
   Note that there are many other opportunities mentioned in the task view, for example the Rccp-based `fastLm()` (found in `{RcppEigen}` e.g.) is very fast as well. Some packages make it possible to handle data which is not in memory. 
   
2. __[Q]{.Q}__: What package implements a version of `match()` that's faster for repeated lookups? How much faster is it?
    
   __[A]{.solved}__: A Google search points use to the `{fastmatch}`-package. We compare it to `base::match()` and observe an impressive relative performance advantage (up to 35x faster).
...
1. __[Q]{.Q}__: What's the difference between `rowSums()` and `.rowSums()`?
   ...
   The internal `.rowSums()` is an entirely different function than the user-facing `rowSums()` function.
    
   Of course (they share the same name!) these functions are heavily related: If we look into the source code of `rowSums()`, we see that it is a wrapper around the internal `.rowSums()`. Just some input validation, conversions and the special cases (complex numbers) are added:
...
2. __[Q]{.Q}__: Make a faster version of `chisq.test()` that only computes the chi-square test statistic when the input is two numeric vectors with no missing values. You can try simplifying `chisq.test()` or by coding from the [mathematical definition](http://en.wikipedia.org/wiki/Pearson%27s_chi-squared_test).
   ...
   We need to check if our new implementation returns the same results.
    ...
    ```{r}
    m_test <- cbind(21:25, seq(21, 29, 2))
