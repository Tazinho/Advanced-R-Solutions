...
1. __<span style="color:red">Q</span>__: Reconstruct the code represented by the trees below:
   ...
   __<span style="color:orange">A</span>__: The source is with you. ;)
...
1. __<span style="color:red">Q</span>__: Construct the expression `if(x > 1) "a" else "b"` using multiple calls to `call2()`. How does the structure code reflect the structure of the AST?
   ...
   __<span style="color:orange">A</span>__: _TODO:_ Exercise got reformulated to use `call2()` instead of `lang()`. Similar to the prefix version we get
...
1. __<span style="color:red">Q</span>__: Compare the ASTs `x + y %+% z` and `x ^ y %+% z`. What have you learned about the precedence of custom infix functions?
