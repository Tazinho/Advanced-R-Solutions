...
2. __[Q]{.Q}__: `=` can also be used in two ways. Construct a simple example that shows both uses.
   ...
   The second way is more explicit, but both return the same syntax tree. When we ignore the braces and compare the trees, we can see, that the first `=` in the second tree is just part of the syntax and the second `=` one is for the usage of assignment.
