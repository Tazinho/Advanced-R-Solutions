...
2. __[Q]{.Q}__: `=` can also be used in two ways. Construct a simple example that shows both uses.
    ...
    <!-- HW: just some notes to point you in the right direction -->
   
    `=` is used both for assignment, and for naming arguments in function calls:
   ...
   So when we play with `ast()`, we can directly see that the following is not possible
    ...
    ast(b = c(c = 1))
    ```
   We get an error, because `b = ` makes R looking for an argument called `a`. Since `x` is the only argument of `ast()`, we get an error.
