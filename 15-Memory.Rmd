...
1.  __<span style="color:red">Q</span>__: If a data frame has one million rows, and three variables (two numeric, and 
    ...
    * a numeric's size is 40 bytes plus 8 bytes per allocated entry.
...
1.  __<span style="color:red">Q</span>__: Which takes up more memory: a factor (`x`) or the equivalent character 
    ...
    * In case of a long vector with only a few levels, the character takes approximately twice the memory of a factor:
    ...
    ```{r}
    # The following two R code examples cannot straightforwardly support above statement.
    object_size(1:20000)
...
1.  __<span style="color:red">Q</span>__: Explain the difference in size between `1:5` and `list(1:5)`.
    ...
    __<span style="color:green">A</span>__: A list with length 0 integer vector needs 88 bytes. For each entry 8 bytes are added (note that for short lists memory is allocated in chunks, similar as explained for integers in the textbook). We can see this via:
    ...
    Since `1:5` needs 72 bytes, `list(1:5)` takes 120 (`= 88 + (72 - 40)`) bytes. So in general the cost for saving atomics within a list is 88 bytes for the list plus 8 bytes per atomic/list entry.
