...
df_new <- tibble::tribble(~ `1 Names and values`, ~ `2 Vectors`, ~ `3 Subsetting`, ~ `4 Functions`, ~ `5 Environments`, ~ `6 Conditions`, ~ `7 Functionals`, ~ `8 Function factories`,  ~ `9 Function operators`,  ~ `10 S3`, ~`11 S4`, ~`12 R6`, ~`13 Expressions`, ~`14 Quasiquotation`, ~`15 Evaluation`, ~`16 Translating R code`, ~ `17 Performance`, ~ `18 Optimising code`, ~ `19 High performance functions with Rcpp`,
                       ...
                       "x",  "x",  "x",   "x",  "o",  "x",  "-",  "-",   NA,   "-",     NA,   NA,  "-",     "x",   "-",   NA,  "-",  "x",  "x",
                       "x",  "x",  "x",   "x",  "x",  "o",  "-",  "-",   NA,   "-",     NA,   NA,  "x",     "-",   "-",   NA,  "-",  "-",  "-",
                       "x",  "x",  "x",   "x",  "-",  "o",  "-",  "-",   NA,   "x",     NA,   NA,  "x",     "x",   "-",   NA,  "-",  "x",   NA,
                       "o",  "x",   NA,   "x",  "x",  "-",  "-",  "-",   NA,   "x",     NA,   NA,  "x",     "x",   "-",   NA,   NA,  "x",   NA,
                       "x",  "x",   NA,   "x",   NA,  "-",  "-",   NA,   NA,   "-",     NA,   NA,  "x",     "x",   "-",   NA,   NA,  "-",   NA,
                       "x",  "x",   NA,   "x",   NA,  "o",  "-",   NA,   NA,   "x",     NA,   NA,  "x",     "x",   "-",   NA,   NA,  "x",   NA,
                        NA,  "x",   NA,   "x",   NA,   NA,  "-",   NA,   NA,   "-",     NA,   NA,  "x",     "-",   "-",   NA,   NA,  "x",   NA,
...
solved_exercises <- ggplot(df_new, aes(x = chapter, fill = solved, group = -id)) +
  ...
  ggtitle(paste0("Solved Exercises: ", round(100 * quote_solutions, 1), "% (2nd Edition)"),
    subtitle = paste0("Solved: ", sum(df_new$solved == "x"),
      "; Started: ", sum(df_new$solved == "o"),
      "; Open: ", length(df_new$solved) - sum(df_new$solved == "x") - sum(df_new$solved == "o")
    )
  )
