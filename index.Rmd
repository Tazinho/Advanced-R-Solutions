...
# import from file --------------------------------------------------------
df_long <- read.table("progress_data.csv", stringsAsFactors = FALSE,
                      sep = "\t", header = TRUE) %>% 
  as_tibble() %>% 
  modify_at("status", factor, levels = c("open", "started", "solved")) %>% 
  arrange(chapter, subchapter, exercise) %>% 
  ...
  mutate(nr = as.integer(nr), 
         ...
         part = factor(part, levels = c("Foundations", "FP", "OOP",
                                        "Metapr.", "Techn."))) %>% 
  # find most recent state for each exercise
  group_by(chapter, subchapter, exercise) %>% 
  filter(updated_when == max(updated_when),
         status != "deleted") %>% 
  ungroup()
