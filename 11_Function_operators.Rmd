...
__[Q1]{.Q}__: Weigh the pros and cons of `download.file %>% dot_every(10) %>% delay_by(0.1)` vs `download.file %>% delay_by(0.1) %>% dot_every(10)`.
...
__[Q2]{.Q}__: Should you memoise `file.download()`? Why/why not?
...
Now, lets check if our `logger()` works as intended and apply it to the `mean()` function:
...
__[A]{.solved}__: To ensure that the function created by `delay_by()` waits that a certain amount of time has passed since it's last execuiton, we incorporate three little changes into our new `delay_atleast()` as indicated in the corresponding comments below.
