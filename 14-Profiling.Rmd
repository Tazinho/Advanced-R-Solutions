...
1.  __<span style="color:red">Q</span>__: List four functions (not just those in base R) that convert a string into a
    ...
    __<span style="color:orange">A</span>__: At least these functions will do the trick: `as.POSIXct()`, `as.POSIXlt()`, `strftime()`, `strptime()`, `lubridate::ymd_hms()`. There might also be some in the timeseries packages `xts` or `zoo`. An update on this will follow...
