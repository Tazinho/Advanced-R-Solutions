...
1.  __<span style="color:red">Q</span>__: What are the alternatives to `optim()`?
    ...
    __<span style="color:green">A</span>__: Depending on the usecase a lot of different options might be considered. For a general overview we would suggest the corresponding [taskview](https://cran.r-project.org/web/views/Optimization.html).
...
1.  __<span style="color:red">Q</span>__: What's the difference between `rowSums()` and `.rowSums()`?
    ...
    Of course (since they have the same name) in this case these functions are heavily related with each other: If we look into the source code of `rowSums`, we see that it is a wrapper around the internal `rowSums`. Just some input checkings, conversionis and the special cases (complex numbers) are added:
