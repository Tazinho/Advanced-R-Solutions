...
1.  __<span style="color:red">Q</span>__: What's the difference between `rowSums()` and `.rowSums()`?
    ...
    Of course (since they have the same name) in this case these functions are heavily related with each other: If we look into the source code of `rowSums`, we see that it is a wrapper around the internal `rowSums`. Just some input checkings and conversionis are added:
