...
1. __<span style="color:red">Q</span>__: Base R contains two function factories, `approxfun()` and `ecdf()`. 
   ...
   __<span style="color:orange">A</span>__: TODO: the answers are partly provided in the "old exercises" subsection below.

2. __<span style="color:red">Q</span>__: Create a function `pick()` that takes an index, `i`, as an argument and returns a function with an argument `x` that subsets `x` with `i`.
...
4. __<span style="color:red">Q</span>__: What happens if you don't use a closure? Make predictions, then verify with the code below.
   ...
   __<span style="color:green">A</span>__: Our new counters aren't too reliable anymore, as they can be overwritten or deleted quite easily:
