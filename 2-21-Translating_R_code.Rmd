...
1. __<span style="color:red">Q</span>__: TODO: Adapt to the new exercise formulation:
...
3. __<span style="color:red">Q</span>__: Reason about the following code that calls `with_html()` referening objects from the environment. Will it work or fail? Why? Run the code to verify your predictions.
   ...
   __<span style="color:orange">A</span>__: (TODO: In the exercise the definition of `p` changed into `p <- function() "p"`. This needs to be added and the exercise/answer might need to get updated) When we created the various HTML tag functions, `address()` was one of them. This HTML tag may be used to provide contact information on an HTML page. All our tag functions are not present in the global environment, but rather elements of the list `html_tags`.
...
4. __<span style="color:red">Q</span>__: Currently the HTML doesn't look terribly pretty, and it's hard to see the
