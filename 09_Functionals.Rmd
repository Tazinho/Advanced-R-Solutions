...
__[A]{.solved}__: There are many ways to visualise this data. However, since there are only 100 data points, we choose a to visualize the distribution via a dot plot. (Unfortunately `ggplot2`s `geom_dotplot()` doesn't compute proper counts as it was created to visualize distribution densities instead of frequencies, so a histogram could be a suitable alternative).
