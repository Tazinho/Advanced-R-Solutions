...
__[A]{.solved}__: There are many ways to visualise this data, but since there are only 100 data points we choose a dot plot to visualize the individual values and the overall distribution. Unfortunately the dot plot doesn't compute proper counts for us (because it was created to visualize distribution densities instead of frequencies), so a histogram could be a suitable alternative.
...
__[A]{.solved}__: `write.csv()` returns `NULL` and we generally call the function for their side-effect (creating a csv-file). So `walk2()` would be the appropriate functional here, otherwise we receive a rather uninformative lists of `NULL`s.
...
__[A]{.solved}__: Our `span_r()` function returns the indices of the (first occurring) longest sequential run of elements where the predicate is true. If the predicate is never true, the longest run has length 0, so we return `integer(0)`.
