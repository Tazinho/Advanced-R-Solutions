...
5.  __<span style="color:red">Q</span>__: `UseMethod()` calls methods in a special way. Predict what the following
    ...
    `h("a")` will return `"char a"`, because `x = "a"` is given as input to the called method, which is of class character `h.character` is called and R also doesn't need to look elsewhere for `x`. 
