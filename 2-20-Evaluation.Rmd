...
5. __[Q]{.Q}__: We can make `base::local()` slightly easier to understand by spreading it over multiple lines:
   ...
   `substitute()` operates (by default) in the current evaluation argument. However, this doesn't matter here, as both, `expr` and `envir` are promises and therefore "... the expression slots of the promises replace the symbols", from `?substitute`. 
...
3. __[Q]{.Q}__: The following function implements the basics of `dplyr::arrange()`. Annotate each line with a comment explaining what it does. Can you explain why `!!.na.last` is strictly correct, but omitting the `!!` is unlikely to cause problems?
   ...
   __[A]{.solved}__: `arrange2()` basically reorders a data frame by one or more of its variables. As `arrange2()` allows to provide the variables as expressions (via `...`), these need to be quoted first. Next they are used to build up an `order()` call, which then gets evaluated inside the data frame. Finally, the data frame gets reordered via integer subsetting. Let's jump into the source code:
   ...
   By using `!!.na.last` the `.na.last` argument is unquoted, when the `order()` call is built. That way, the `na.last` argument is already correctly specified (typically `TRUE`, `FALSE` or `NA`).
