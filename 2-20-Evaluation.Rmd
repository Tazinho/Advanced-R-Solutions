...
5. __[Q]{.Q}__: We can make `base::local()` slightly easier to understand by spreading it over multiple lines:
   ...
   From now on I don't understand: When we are in the global environment and evaluate the call. Where does it get evaluated? Obviously inside of `new.envir()`. However, when the expressions are not found there, it is more of a question, whis environment is the parent environment of `new.envir`. The documentation `new.envir()` and also the default argument (`parent.frame()`) suggest that it is `parent.frame()`, so the global environment (this also makes completely sense, as I tested that local finds variables in the global environment, but none which are only provided in `local3()`'s execution environment). However, when I manually check and add `env_parent(envir)` inside `local3()` I always get identical(env_parent(envir), environment()), which IMO suggests, that the parent of `envir` is the execution environment of `local3()` and the global environment is only the grandparent.
   
   Also I don't understand these older bullet points (I just keep them in for now in case they help you, Henning/Hadley?)
