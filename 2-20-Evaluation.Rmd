...
1. __[Q]{.Q}__: Carefully read the documentation for `source()`. What environment does it use by default? What if you supply `local = TRUE`? How do you provide a custom environment?
   ...
   __[A]{.solved}__: By default, `source()` uses the global environment. A specific evaluation environment may be chosen, by passing it to the `local` argument. To use a local environment set `local = TRUE`.
...
3. __[Q]{.Q}__: Fill in the function bodies below to re-implement `get()` using `sym()` and `eval()`, and`assign()` using `sym()`, `expr()`, and `eval()`. Don't worry about the multiple ways of choosing an environment that `get()` and `assign()` support; assume that the user supplies it explicitly
   ...
   __[A]{.solved}__: We reimplement these two functions using tidy evaluation. First expression is build, which is then evaluated.
   ...
   The implementation would be even more concise, if the user provided an expression instead of a string:
...
5. __[Q]{.Q}__: We can make `base::local()` slightly easier to understand by spreading it over multiple lines:
   ...
   By default `substitute()` operates in the current evaluation argument. However, this doesn't matter here, as both, `expr` and `envir` are promises and therefore "the expression slots of the promises replace the symbols", from `?substitute`.
   
   Next, `call` will be evaluated in the global environment. 
   
   From now on I don't understand what happens exactly: When we are in the global environment and evaluate the call. Where does it get evaluated? Obviously inside of `new.envir()`. However, when the expressions are not found there, it is more of a question, which environment is the parent environment of `new.envir()`. The documentation of `new.envir()` and also the default argument (`parent.frame()`) suggest that it is `parent.frame()`, so the global environment (this also makes completely sense, as in some tests I did `local3()` finds variables in the global environment, but it doesn't find variables when provided in `local3()`'s execution environment). However, when I manually check and add `env_parent(envir)` inside `local3()` I get `identical(env_parent(envir), environment())` is `TRUE`, which IMO suggests, that the parent of `envir` is the execution environment of `local3()` and the global environment is only the grandparent.
   
   Also I don't understand these older bullet points (I just keep them in for now in case they help you. What do you think? Henning/Hadley?)
...
1. __[Q]{.Q}__:  I've included an alternative implementation of `threshold_var()` below. What makes it different to the approach I used above? What makes it harder?
   ...
   We can see, that `threshold_var2()` no longer coerces the symbol to a string. Therefore `$` instead of `[[` can be used for subsetting. Initially we suspected partial matching to work with `$`, but this seems to avoided, when the expression is tidily evaluated.
