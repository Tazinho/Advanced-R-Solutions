...
5.  __<span style="color:red">Q</span>__: Write an enhanced version of `str()` that provides more information 
    ...
    Note that we wanted to have non standard evaluation like the original `str()` function. Therefore we used `expr_text()` from the [lazyeval](https://github.com/hadley/lazyeval) package. As a result, `fstr(packagename::functionname)` will result in an error in contrast to `str()`.
