...
1.  __<span style="color:red">Q</span>__: Implement your own version of `compose()` using `Reduce` and `%o%`. For 
    ...
    ```{r, eval}
    compose_red <- function(...) {
      Reduce(function(f, g) function(...) f(g(...)), list(...))
    }
    compose_red(mean, length, unique)(1:10)
    
    compose_red_bonus <- function(...) {
      Reduce(partial(partial(`%o%`)), list(...))
    }
    compose_red_bonus(mean, length, unique)(1:10)
    ```
