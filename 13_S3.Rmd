...
After `unclass()` has removed the class attribute from `some_date`, so that the default method is chosen. `mean.default(unclass(some_days))` then calculates the mean of the underlying double.
...
__[A]{.solved}__: This code returns an object of the class "ecdf" with the superclasses "stepfun" and "function" and contains an empirical cumulative distribution function of its input. The ecdf object is built on the base type "closure" (a function) and the expression, which was used to create it (`rpois(100, 10)`) is stored in in the `call` attribute.
...
__[A]{.solved}__: This code returns a "table" object, which is build upon the "integer" type. The attribute "dimnames" is used to name the elements of the integer vector.
...
__[Q5]{.Q}__: Read the documentation for `utils::as.roman()`. How would you write a constructor for this class? Does it need a validator? What would a helper look like?
...
Note that we can get a more complete overview if we work with the output of ` s3_methods_generic("[")`.
...
s3_methods_generic("[") %>% 
  filter(visible) %>% 
  mutate(method = paste0("[.", class)) %>% 
  mutate(argnames = purrr::map(method, ~ names(formals(.x)))) %>% 
  mutate(args = purrr::map(method, ~ formals(.x))) %>% 
  mutate(args = purrr::map2(argnames, args,
                            ~ paste(.x, .y, sep = " = "))) %>% 
  mutate(args = purrr::set_names(args, method)) %>% 
  pull(args) %>% 
  head()
```
...
__[Q1]{.Q}__: Categorise the objects returned by `lm()`, `factor()`, `table()`, `as.Date()`, `ecdf()`, `ordered()`, `I()` into the styles described above.
...
__[A]{.solved}__: `class()` returns `"integer"` in both cases. But the class of `x1` is created implicitly and inherits from the `"numeric"` class, while the class of `x2` is set explicitly and no parent class is assumed.
...
The output of `s3_dispatch()` shows us, that setting the class explicitly for `x2` avoids considering the implicit parent class `"numeric"` for method dispatch. We can also see that various of the possible methods are not implemented. Rather an internal dispatch (written in C) is used (highlighted via "=>"), which shows that `[` is an internal generic.
