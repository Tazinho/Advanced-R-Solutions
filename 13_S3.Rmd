...
The `factor()` helper including the constructor (`new_factor()`) and its validator (`validate_factor()`) were given in Advanced R. However, as the goal of this question is to through an early error within the helper, we only repeat the code for the helper:
...
For a better understanding of `s3_dipatch()`'s output we quote from `?sd_dispatch`:
