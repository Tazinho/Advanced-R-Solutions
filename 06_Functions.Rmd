...
__[Q2]{.Q}__: In the following call, explain how to find the documentation for the named arguments in the following function call:
...
__[A]{.solved}__: `in_dir()` takes *a path* to a working directory as an argument. First the working directory is changed accordingly. `on.exit()` ensures that the modification to the working directory is reset to the initial value when the function exits. `in_dir()` expresses the change of the working directory very explicitly. It also gives the user full control over the location of the directory to execute the code in.

In `source()` the `chdir` argument specifies if the working directory should be changed to the directory containing `file` (which in this case has to be a pathname). The default for this *logical value* is `FALSE`, so you don't have to provide a value but you are also less flexible in choosing the execution working directory.
