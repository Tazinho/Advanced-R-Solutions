...
__[A]{.solved}__: `in_dir()` takes as first argument (`dir`) a path for a working directory in which the provided code (`code`) should be executed. Therefore, the current working directory is first changed in `in_dir()` via `setwd()`. Next, `on.exit()` ensures that the working directory is reset after the `code` has been executed (or thrown an error). By passing the path explicitly, the user has full control over the directory to execute the code in.

In `source()` the code to be executed is passed via the `file` argument. The `chdir` argument allows to execute this code in the same directory where `file` is located (`chdir = TRUE`). The default for `chdir` is `FALSE`, so you don't have to provide a value. However, as you can only provide `TRUE` or `FALSE`, you are also less flexible in choosing the working directory for the code execution.
