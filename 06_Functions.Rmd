...
The function strives to 'write output as far as possible to file', which is why an error handling is needed. This is achieved by first quoting the expressions (via `substitute()`) and then evaluating them step-by-step (via `lapply()` and a variant of `eval`), which makes the function more robust.
