...
1. __[Q]{.Q}__: Given a name, like `"mean"`, `match.fun()` lets you find a function. Given a function, can you find its name? Why doesn't that make sense in R?
   ...
   The first function is not bound to a name while multiple names (`f1`, `f2` and `f3`) point to the second function. Also, each function has its own environment - so it's possible that two functions may have the same "code", but are not identical because they have different environments (or closures). Lastly, anonymous functions don't have names so we have no way to look these up.

   For functions in the global environment we *could* find their names, by inspecting the function body. But this approach boils down to deparsing the function into a string and comparing these values.

    ```{r}
    funs_global <- mget(lsf.str())

    names(which(sapply(funs_global, function(x) body(x) == body(f1))))
    ```

   The main point is that the relation between name and object is only clearly defined in one direction in R. There's no efficient way to go backwards. This is true for all values, not just functions.
