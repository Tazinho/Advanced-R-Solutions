...
4.  __<span style="color:red">Q</span>__: What happens if you use `quote()` instead of `substitute()` inside of
    ...
    __<span style="color:orange">A</span>__: We will get ```"Error in eval(expr, envir, enclos) : object 'x' not found"``` in the above example. We were not sure if the problem relates to finding the right expression or the correct environment for evaluation. But fortunately we can find some more information in another [repository](https://github.com/asnr/advanced-r-answers/blob/master/metaprogramming/nse.md):
    ...
    subset2 <- function(x, condition) {
      ...
      r <- eval(condition_call, x, enclos = NULL)
      x[r, ]
    }
    ...
    We can see that this time R is looking for the symbol `condition` and can't find it. In the default case (`enclos = parent.frame()`), where R is looking for `x`, the promise related to `x > 8` was already recognized by R. So in both `quote()`-cases, `x > 8` is not found by R, when `condition_call` should be evaluated in `x`. So in the first case (default), R looks in `eval`'s calling environment, and finds the promise `x > 8`. So it looks in the global environment to evaluate it. In the other case (`enclos = NULL`) `condition` is not found in `x` (the environment) and R is not looking in further environemts, since `enclos = NULL`. So the search is already finished before the condition related to `condition call` is even found.
    
    This is opposed to `substitute()`, which isn't only capturing the symbol `condition`, but the expression slot of the condition promise object. (Still need to show this!)
