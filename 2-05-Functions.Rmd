...
6. __<span style="color:red">Q</span>__: How many arguments are required when calling `library()`?
   ...
   __<span style="color:green">A</span>__: `library()` doesn't require any arguments. When called without arguments `library()` (invisibly) returns a list of class libraryIQR, containing a header and footer element as well as a results matrix, with one row and three columns per installed package. These columns contain entries for the name of the package ("Package"), the path to the package ("LibPath") and the title of the package ("Title"). `library()` also got it's own print method (`print.libraryIQR`) which shows the information in it's own window in the regarding development environment (i.e. RGui or RStudio).
   ...
   However, since the `package` and `help` argument from `library()` do not show a default value, the possibility to call `library()` without arguments might be hard to spot for users who are not too familiar with the details of the function during their first pass over the function.

    ```{r}
    str(formals(library))
    ```
    
   Instead, `library()` terminates internally via the `missing()` function, if these arguments were supplied.
...
1. __<span style="color:red">Q</span>__: Explain the following results:
   ...
   In contrast, the generic function `mean()` expects `x`, `trim`, `na.rm` and `...` for it's default method, `mean.default()`, which handles numeric and logical vector input for it's `x` argument (see `?mean`).
...
1. __<span style="color:red">Q</span>__: Rewrite the following code snippets into prefix form:
   ...
   __<span style="color:green">A</span>__: In the following, we rewrite the exprossions to match the exact syntax from above. Since prefix functions have the same operator precedence as brackets, we can omit the brackets in the second expression and provide the regarding alternative version as a comment.
