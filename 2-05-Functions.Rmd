...
2. __<span style="color:red">Q</span>__: It’s possible (although typically not useful) to call an anonymous function. Which of the two approaches below is correct? Why?
   ...
   The anonymous function `function(x) 3` is surrounded by a pair of parentheses before it is called by `()`. These extra parentheses seperate the function call from the anonymous functions body. Without these a function with the invalid body `3()` is returned, which throws an error when we call it:
...
7. __<span style="color:red">Q</span>__: When does printing a function not show what environment it was created in?  
   ...
   __<span style="color:green">A</span>__: Only primitive functions or functions created in the global environment do not show the enivronment they were created in when they are printed.
...
1. __<span style="color:red">Q</span>__: What important property of `&&` makes `x_ok()` work?
   ...
   The desired behaviour is reached by combining the assertions through `&&` instead of `&`. `&&` does not perform elementvise comparisons, instead it uses the first element of each value only. It also uses lazy evaluation, in the sense that evaluation "proceeds only until the result is determined" (from `?Logic`).
...
5. __<span style="color:red">Q</span>__: Explain why this function works. Why is it confusing?
   ...
   This function is confusing, because its behaviour changes when `x`'s value is supplied directly. Now the value from the calling environment will be used and the overwriting of `stop` won't affect the outcome anymore.
...
6. __<span style="color:red">Q</span>__: How many arguments are required when calling `library()`?
   ...
   Because the `package` and `help` argument from `library()` do not show a default value, it's easy to overlook the possibility to call `library()` without these arguments. (Instead of providing `NULL`s as default values `library()` uses `missing()` to check if these argements were provided.)
...
1. __<span style="color:red">Q</span>__: What does `load()` return? Why don’t you normally see these values?
   ...
   __<span style="color:green">A</span>__: `load()` reloads datasets which were previously written to disk via `save()`. When run succesfully, `load()` invisibly returns a character vector, containing the names of the newly loaded objects. To print these names to the console, one can set the argument `verbose` to `TRUE` or surround the call in parentheses to trigger R's autoprinting mechanism.
...
2. __<span style="color:red">Q</span>__: Clarify the following list of odd function calls:
   ...
   __<span style="color:green">A</span>__: None of these functions provides a `...` argument. Therefore the function arguments are first machted exactly, then via partial matching and finally by position. This leads us to the following explicit function calls:
...
3. __<span style="color:red">Q</span>__: Explain why the following code fails:
   ...
   The error occures during the assignment, because no corresponding replacement function, i.e. `get<-` exists for `get()`. To confirm this we can reproduce the error via the following simple example.
