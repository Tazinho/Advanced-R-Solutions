...
6. __<span style="color:red">Q</span>__: How many arguments are required when calling `library()`?
   ...
   However, for users who are not that familiar with the details of `library()` this behaviour might be hard to spot on first pass pass over the function, since the `package` and `help` argument from `library()` do not show a default argument.
