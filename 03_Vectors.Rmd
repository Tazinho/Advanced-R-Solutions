...
__[A]{.solved}__: When the levels of a factor ar modified the underlying integer values will remain unchanged. But when you print the factor, its order will be the changed because the mapping of the integers to their levels has been altered.
...
__[A]{.solved}__: Both `f1` and `f2` show an inverted order of `factor(letters)`. For `f2` this has been achieved by reversing the entire factor. This step alters the order of the elements, but leaves the order of the levels untouched.

For `f3` the factor-levels are explicitly reversed, when the factor is created. This leaves the order of the elements intact and just changes the factor levels. The underlying integers are created in the appropriate order.
...
A package that deals with these kind of problems in more depth and provides a structural solution for them is the [`{vctrs}` package](https://github.com/r-lib/vctrs) [@vctrs] which is also used throughout the tidyverse [@tidyverse].
