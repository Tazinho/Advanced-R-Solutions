...
- `is.vector()` tests if an object is a vector (as defined in Advanced R) and has no attributes, apart from names or if it is an expression.
...
* Subsetting with out-of-bounds and `NA` values leads to different output. E.g. for `[` we get `NA` for atomics and `NULL` for lists. (This will be described in more detail within the [subsetting chapter](https://adv-r.hadley.nz/subsetting.html).)
