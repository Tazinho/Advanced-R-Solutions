...
__[A]{.solved}__: In R scalars are represented as vectors of length one. However, there is no way to create a literal raw scalar directly; you have to create a vector first. Then you can easily create one from numeric or character values.
...
For complex numbers real and imaginary parts may be provided directly to the `complex()` constructor.
...
__[A]{.solved}__: These are all "one dimensional". If you imagine a 3d cube, `x1` is the x-dimension, `x2` is the y-dimension, and `x3` is the z-dimension.
...
- Atomic vectors point to one address in memory, while lists contain a separate references for each element. (This was described in the list sections of the [vectors](https://adv-r.hadley.nz/vectors-chap.html#lists) and the [names and values](https://adv-r.hadley.nz/names-values.html#list-references) chapters.)
...
- Subsetting with out-of-bound and `NA` values leads to different output. E.g. for `[` we get `NA` for atomics and `NULL` for lists. (This will be described in more detail within the [subsetting chapter](https://adv-r.hadley.nz/subsetting.html).)
...
# Internal representasions
unclass(date)
unclass(dttm_ct)
```

Combining these objects via the `c()` leads to surprising results. This is because the `c()` generic does only dispatch on it's first argument:
...
Data frames, however, require unique rownames and you get different results depending on how you attempt to set them. If you use `row.names()` directly, you 
get an error:
...
__[A]{.solved}__: The type of the result of `as.matrix` can vary and depends on the types of the input columns (see `?as.matrix`):
...
We can illustrate and compare the mechanics of these functions for a concrete an example. `as.matrix()` makes it possible to retrieve most of the original information from the dataframe, but leaves us with characters. To retrieve all the information from the `data.matrix()` output, we would need a mapping table for each column.
