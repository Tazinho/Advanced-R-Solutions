...
We can illustrate and compare the mechanics of these functions for a concrete example. `as.matrix()` makes it possible to retrieve most of the original information from the data frame, but leaves us with characters. To retrieve all information from `data.matrix()`'s output, we would need a lookup table for each column.
