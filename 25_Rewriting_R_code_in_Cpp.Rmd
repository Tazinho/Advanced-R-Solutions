...
We also extend `anyC` so it can deal with missing values. Please note that this introduces some code duplication. This could be avoided by moving the check for missing values to the inner loop at the expense of a slight decrease of performance. Here we use `LogicalVetor` as return type. If we would use `bool` instead, the C++ `NA_LOGICAL` would be converted into R's logical `TRUE`.
...
__[A]{.solved}__: Our `NA`-aware `cumsumC()` function will return a vector of the same length as `x`. By default (`na_rm = false`) all values following the first `NA` input value will be set to `NA`, because they depend on the unknown missing value. In case of `na_rm = true` the `NA` values are treated like zeros.
...
// [[Rcpp::export]]
NumericVector diffC2(NumericVector x, int lag = 1,
                    bool na_rm = false){
  int n = x.size();
