...
We can see, that `threshold_var2()` no longer coerces the symbol to a string. Therefore `$` instead of `[[` can be used for subsetting. Initially we suspected partial matching would be intoduced by `$`, but `.data` deliberately avoids this problem.
