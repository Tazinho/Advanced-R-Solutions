...
__[A]{.solved}__: `ifelse()`'s arguments are named `test`, `yes` and `no`. It returns the entry for `yes` when `test` is `TRUE`, the entry for `no` when `test` is `FALSE` and `NA` when `test` is `NA`. Therefore, the expressions above return vectors of type `double` (`1`), `character` (`"no"`) and `logical` (`NA`).

Additionally, `ifelse()` has some special characteristics. E.g. it's vectorised over all its arguments. If `yes` or `no` are shorter than `test`, they are recycled. Otherwise, their additional elements will be ignored. Further, the documentation says about the output type:
