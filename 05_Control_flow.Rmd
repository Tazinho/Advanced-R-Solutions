...
This is surprising because it uses the type of `test`. In practice this means, that if `test` is neither `TRUE` nor `FALSE`, `test` is simply converted to logical.
...
__[A]{.solved}__: In this loop `x` takes on the values of the initial `xs` (`1`, `2` and `3`), indicating that it is evaluated just once in the beginning of the loop, not after each iteration. (Otherwise we would run into an infinite loop here.)
