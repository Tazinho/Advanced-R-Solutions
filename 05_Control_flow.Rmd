...
__[A]{.solved}__: The arguments of `ifelse()` are named `test`, `yes` and `no`. `ifelse()` returns the entry for `yes` when `test` is `TRUE`, the entry for `no` when `test` is `FALSE` and `NA` when `test` is `NA`. This is surprising because it uses the type of `test`. Therefore, the expressions above return vectors of type `double` (`1`), `character` (`"no"`) and `logical` (`NA`).

Additionally, `ifelse()` has some special characteristics. E.g. it is vectorised over all its arguments. If `yes` or `no` are shorter than `test`, they are recycled. Otherwise, their additional elements are ignored. Further, the documentation says about the output type:
