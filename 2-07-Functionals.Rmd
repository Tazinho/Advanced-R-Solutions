...
2. __<span style="color:red">Q</span>__: What's the relationship between `which()` and `Position()`? What's the relationship between `where()` and `Filter()`?
   ...
   is useful to return a logical vector from a condition asked on elements of a list or a data frame. `Filter(f, x)` returns all elements of a list or a data frame, where the supplied predicate function returns `TRUE`. So the relation is
`Filter(f, x) <=> x[where(f, x)]`.

3. __<span style="color:red">Q</span>__: `simple_reduce()` has a problem when `x` is length 0 or length 1. Describe the source of the problem and how you might go about fixing it.
...
4. __<span style="color:red">Q</span>__: Implement the `span()` function from Haskell: given a list `x` and a predicate function `f`, `span()` returns the location of the longest sequential run of elements where the predicate is true. (Hint: you might find `rle()` helpful.)
...
6. __<span style="color:red">Q</span>__: The function below scales a vector so it falls in the range [0, 1]. How would you apply it to every column of a data frame? How would you apply it to every numeric column in a data frame?
...
2. __<span style="color:red">Q</span>__: What do `eapply()` and `rapply()` do? Does purrr have equivalents?
   ...
   __<span style="color:green">A</span>__: (Maybe this old ex. is related!? There's no equivalent to `split()` + `vapply()`. Should there be? When would it be useful? Implement one yourself.)
