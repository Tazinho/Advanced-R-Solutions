...
1. __<span style="color:red">Q</span>__: Create a bank account R6 class that stores a balance and allows you to deposit and withdraw money. Create a subclass that throws an error if you attempt to go into overdraft. Create another subclass that allows you to go into overdraft, but charges you a fee.
   ...
   Finally, we creat a class that charges a constant fee (of 1) for each withdraw which leaves the account with a negative balance.
   ...
   And provide a regarding test, which should result in -12, since we pay the fee two twice:
...
4. __<span style="color:red">Q</span>__: Can subclasses access private fields/methods from their parent? Perform an experiment to find out.
   ...
   __<span style="color:green">A</span>__: Our experiment shows that only methods can be accessed by the subclass
...
1. __<span style="color:red">Q</span>__: Create a class that allows you to write a line to a specified file.  You should open a connection to the file in `$initialize()`, append a line using `cat()` in `$append_line()`, and close the connection in `$finalize()`.
   ...
   __<span style="color:green">A</span>__: (TODO: create a temporary file below and prove that this is working... also explain what `open = "a"` does (found it on https://stackoverflow.com/questions/17710469/why-wont-cat-append-to-a-file-connection) and explain that `cat` usually appends for files but not with connections (except `open = "a"`...). Afterwards set eval to TRUE in the code chunk.)
