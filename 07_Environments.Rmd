...
__[A]{.solved}__: At creation time each function binds its parent environment: `f1` binds the global environment, `f2` binds the environment of `f1` and so on. The function environments contain functions and the values provided in the function call.

We can also inspect the binding of the environments, by printing their function environments (as well as the parent environments).
...
We can visualize these relations in the following diagram.
