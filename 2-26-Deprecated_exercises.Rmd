...
1. [Q]{.Q}: What does `options(error = recover)` do? Why might you use it?

   [A]{.solved}: In case of `options(error = recover)` `utils::recover()` will be called (without arguments) in case of an error. This will print out a list of calls which precede the error and lets the user choose to incorporate `browser()` directly in any of the regarding environments allowing a practical mode for debugging.

2. [Q]{.Q}: What does `options(error = quote(dump.frames(to.file = TRUE)))` do? Why might you use it?

   [A]{.solved}: This option writes a dump of the evaluation environment where an error occurs into a file ending on `.rda`. When this option is set, R will continue to run after the first error. To stop R at the first error use `quote({dump.frames(to.file=TRUE); q()})`. These options are especially useful for debugging non-interactive R scripts afterwards ("post mortem debugging").

## Expressions (new)

1.  [Q]{.Q}: `base::alist()` is useful for creating pairlists to be used for function arguments:
...
1.  [Q]{.Q}: Why are the following two invocations of `lapply()` equivalent?
    ...
    [A]{.solved}: In the first statement each element of `trims` is explicitly supplied to `mean()`'s second argument. In the latter statement this happens via
positional matching, since `mean()`'s first argument is supplied via name
in `lapply()`'s third argument (`...`).

2.  [Q]{.Q}: The function below scales a vector so it falls in the range [0, 1]. How
    ...
    [A]{.solved}: Since this function needs numeric input, one can check this via an if clause. If one also wants to return non-numeric input columns, these can be supplied to the `else` argument of the `if()` "function":
    
    ```{r, eval = FALSE}
    data.frame(lapply(iris, function(x) if (is.numeric(x)) scale01(x) else x))
    ```

3.  [Q]{.Q}: Use both for loops and `lapply()` to fit linear models to the
    ...
    [A]{.solved}: Like in the first exercise, we can create two `lapply()` versions:
...
4.  [Q]{.Q}: Fit the model `mpg ~ disp` to each of the bootstrap replicates of `mtcars`
...
5.  [Q]{.Q}: For each model in the previous two exercises, extract $R^2$ using the
    function below.
    ...
    [A]{.solved}: For the models in exercise 3:
...
1.  [Q]{.Q}: Use `vapply()` to:
    ...
    [A]{.solved}: As a numeric `data.frame` we choose `cars`:
...
2.  [Q]{.Q}: Why is using `sapply()` to get the `class()` of each element in
    a data frame dangerous?
    
    [A]{.solved}: Columns of data.frames might have more than one class, so the class of `sapply()`'s output may differ from time to time (silently). If ...
...
3.  [Q]{.Q}: The following code simulates the performance of a t-test for non-normal
...
4.  [Q]{.Q}: What does `replicate()` do? What sort of for loop does it eliminate? Why
    do its arguments differ from `lapply()` and friends?
    
    [A]{.solved}: As stated in `?replicate`:
...
5.  [Q]{.Q}: Implement a version of `lapply()` that supplies `FUN` with both the name
...
6.  [Q]{.Q}: Implement a combination of `Map()` and `vapply()` to create an `lapply()`
    ...
    [A]{.solved} As we understand this exercise, it is about working with a list of lists, like in the following example:
...
7.  [Q]{.Q}: Implement `mcsapply()`, a multi-core version of `sapply()`. Can you
    implement `mcvapply()`, a parallel version of `vapply()`? Why or why not?

### Manipulating matrices and data frames

1.  [Q]{.Q}: How does `apply()` arrange the output? Read the documentation and perform
    some experiments.
...
2.  [Q]{.Q}: There's no equivalent to `split()` + `vapply()`. Should there be? When
    would it be useful? Implement one yourself.
    
    [A]{.solved}: We can modify the `tapply2()` approach from the book, where `split()` and `sapply()` were combined:
...
3.  [Q]{.Q}: Implement a pure R version of `split()`. (Hint: use `unique()` and
...
4.  [Q]{.Q}: What other types of input and output are missing? Brainstorm before you look up some answers in the [plyr paper](http://www.jstatsoft.org/v40/i01/).
    
    [A]{.solved}: From the suggested plyr paper, we can extract a lot of possible combinations and list them up on a table. Sean C. Anderson already has done this based on a presentation from Hadley Wickham and provided the following result [here](http://seananderson.ca/2013/12/01/plyr.html).
...
1.  [Q]{.Q}: Why isn't `is.na()` a predicate function? What base R function is closest
    to being a predicate version of `is.na()`?
    
    [A]{.solved}: Because a predicate function always returns `TRUE` or `FALSE`. `is.na(NULL)` returns `logical(0)`, which excludes it from being a predicate function. The closest in base that we are aware of is `anyNA()`, if one applies it elementwise.

2.  [Q]{.Q}: Use `Filter()` and `vapply()` to create a function that applies a summary
...
3.  [Q]{.Q}: What's the relationship between `which()` and `Position()`? What's
    the relationship between `where()` and `Filter()`?
    
    [A]{.solved}: `which()` returns all indices of true entries from a logical vector. `Position()` returns just the first (default) or the last integer index of all true entries that occur by applying a predicate function on a vector. So the default relation is `Position(f, x) <=> min(which(f(x)))`.
...
4.  [Q]{.Q}: Implement `Any()`, a function that takes a list and a predicate function,
...
5.  [Q]{.Q}: Implement the `span()` function from Haskell: given a list `x` and a
    ...
    [A]{.solved}: Our `span_r()` function returns the first index of     the longest sequential run of elements where the predicate is true. In case of more than one longest sequenital, more than one first_index is returned.
...
1.  [Q]{.Q}: Implement a summary function that works like `base::summary()`, but uses a
    list of functions. Modify the function so it returns a closure, making it 
    possible to use it as a function factory.

1.  [Q]{.Q}: Which of the following commands is equivalent to `with(x, f(z))`?
...
1.  [Q]{.Q}: Implement `arg_max()`. It should take a function and a vector of inputs,
...
2.  [Q]{.Q}: Challenge: read about the
...
1.  [Q]{.Q}: Implement `smaller` and `larger` functions that, given two inputs, return
...
2.  [Q]{.Q}: Create a table that has _and_, _or_, _add_, _multiply_, _smaller_, and
    ...
    [A]{.solved} In the following table we can see the requested base R functions, that we are aware of:
...
3.  [Q]{.Q}: How does `paste()` fit into this structure? What is the scalar binary
    ...
    [A]{.solved} `paste()` behaves like a mix. If you supply only length one arguments, it will behave like a reducing function, i.e. :
...
1. [Q]{.Q}: The most important S3 objects in base R are factors, data frames, difftimes, and date/times (Dates, POSIXct, POSIXlt). You've already seen the attributes and base type that factors are built on. What base types and attributes are the others built on?
    
   [A]{.started}: TODO: Add answer for difftime.
...
1.  [Q]{.Q}: Draw a Venn diagram illustrating the relationships between functions, generics, and methods.
    
    [A]{.started}: Funtions don't have to be generics or methods, but both the latter are functions. It is also possible that a function is both, a method and a generic, at the same time, which seems to be relatively awkward, so that also the author of the textbook doesn't recommend it, see `?pryr::ftype`
...
2.  [Q]{.Q}: Write a constructor for `difftime` objects. What base type are they built on? What attributes do they use? You'll need to consult the documentation, read some code, and perform some experiments.
    
    [A]{.solved}: Our constructor should be named `new_class_name`, have one argument for its base type and each attribute and check the base types of these arguments as well.
...
### Inheritance
1. [Q]{.Q}: The `ordered` class is a subclass of `factor`, but it's implemented in a very ad hoc way in base R. Implement it in a principled way by building a constructor and providing `vec_restore()` method.
   ...
   [A]{.started}: TODO: the olad exercise text ended on "an `as_ordered` generic". Check the answer if it needs to be updated.
...
2. [Q]{.Q}: What's the difference between the generics generated by these two calls?
   ...
   [A]{.solved}: The first call defines a standard generic and the second one creates a nonstandard generic. One can confirm this directly whlie printing (showing in S4 jargon) the function.
...
1. [Q]{.Q}: Use `ast()` and experimentation to figure out the three arguments to an `if()` call. What would you call them? Which arguments are required and which are optional?
    
   [A]{.solved}: You can write an `if()` statement in several ways: with or without `else`, formatted or in one line and also in prefix notation. Here are several versions focussing on the possibility of leaving out curly brackets.
...
2. [Q]{.Q}: What are the arguments to the `for()` and `while()` calls?

   [A]{.solved}: `for()` requires an *index* (called `var` in the docs), a *sequence* and an *expression*, for example
...
3. [Q]{.Q}: Two arithmetic operators can be used in both prefix and infix style.
   What are they?
    
   [A]{.solved}: I am not sure how this is meant to be. Theoretically every arithmetic operator can be written in prefix notation via backticks. On the other hand, `+` and `-` seem to be the only ones, which can be written in infix notation without backticks.
...
1.  [Q]{.Q}: Why does `as.Date.default()` use `substitute()` and `deparse()`? Why does `pairwise.t.test()` use them? Read the source code.

    [A]{.solved}: `as.Date.default()` uses them to convert unexpected input expressions (neither dates, nor `NAs`) into a character string and return it within an error message.
...
2.  [Q]{.Q}: What does the following function do? What would be a good name for it?
    ...
    [A]{.solved}: It returns a new version of the inputfunction. That version will always return the result of it's first run (in case this not `NULL`), no matter how the input changes. Good names could be `first_run()` or `initial_return()`.

3.  [Q]{.Q}: Modify `delay_by()` so that instead of delaying by a fixed amount of time,
    ...
    [A]{.solved}:
4.  [Q]{.Q}: Write `wait_until()` which delays execution until a specific time.
...
5.  [Q]{.Q}: There are three places we could have added a memoise call: why did we
    ...
    [A]{.solved}: The second was chosen. It's easy to see why, if we eliminate the other two options:
...
6.  [Q]{.Q}: Why is the `remember()` function inefficient? How could you implement it
    in more efficient way?

7.  [Q]{.Q}: Why does the following code, from
    ...
    [A]{.solved}: You can read in the [stackoverflow](http://stackoverflow.com/questions/8440675) link that the question arose, because the original return of
...
1.  [Q]{.Q}: Create a `negative()` FO that flips the sign of the output of the
...
2.  [Q]{.Q}: The `evaluate` package makes it easy to capture all the outputs (results,
    ...
    [A]{.solved}: One way is just to capture the output of  `tryCatch()` with identity handlers for errors and warnings:
...
3.  [Q]{.Q}: Create a FO that tracks files created or deleted in the working directory
...
1.  [Q]{.Q}: Our previous `download()` function only downloads a single file. How can
...
2.  [Q]{.Q}: Read the source code for `plyr::colwise()`. How does the code work? What
    ...
    [A]{.started}: We describe how it works by commenting the source code:
...
3.  [Q]{.Q}: Write FOs that convert a function to return a matrix instead of a data
...
4.  [Q]{.Q}: You've seen five functions that modify a function to change its output
...
5.  [Q]{.Q}: Look at all the examples of using an anonymous function to partially
    ...
    [A]{.solved}: The results are easy to read. Especially the `Map()` examples profit in readability:
...
1.  [Q]{.Q}: Implement your own version of `compose()` using `Reduce` and `%o%`. For
    bonus points, do it without calling `function`.
    
    [A]{.solved}: We use the definition from the textbook:
...
2.  [Q]{.Q}: Extend `and()` and `or()` to deal with any number of input functions. Can
    you do it with `Reduce()`? Can you keep them lazy (e.g., for `and()`, the 
    function returns once it sees the first `FALSE`)?
    
    [A]{.solved}: We use `and()` and `or()` as defined in the textbook. They are lazy, since they are build up on `&&` and `||`. Also their reduced versions stay lazy, as we will show at the end of the code
...
3.  [Q]{.Q}: Implement the `xor()` binary operator. Implement it using the existing
    ...
    [A]{.started}: Both versions are implemented straight forward, as also the reduced versions. However, the parallel versions need a little bit more care:
...
4.  [Q]{.Q}: Above, we implemented boolean algebra for functions that return a logical
...
1. [Q]{.Q}: How is `rlang::maybe_missing()` implemented? Why
does it work?

   [A]{.solved}: Let us take a look at the functions source code to see what's going on
...
1. [Q]{.Q}: `deparse()` produces vectors when the input is long. For example, the following call produces a vector of length two:
