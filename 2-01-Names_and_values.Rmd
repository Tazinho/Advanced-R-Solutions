...
3. __<span style="color:red">Q</span>__: Predict the output of the following code:
   ...
   __<span style="color:green">A</span>__: To predict the object sizes let's first find out how much memory an empty integer occupies and how much the memory size grows with the length of the vector.
   
    ```{r}
    # Memory needed for a 0-length integer
    obj_size(integer(0))
    
    # Additional memory per element
    diff(sapply(0:64, function(x) obj_size(integer(x))))
    ```
    
   From the first line of the code chunk above we know that R needs 48 bytes (abbreviated in calculations as B) for an empty integer. From the second line we can see that R adds 4 bytes per additional element. To explain the small irregularities a bit more in depth, we quote Hadley's explanation from the first edition of Advanced R:
   ...
   Now, the first part is straight forward, as we just need to calculate 48 B + 1000000 * 4 B = 4000048 B ~ 4 MB for our prediction.
   ...
   To predict the size of `y <- list(x, x)` consider that both list elements of `y` point to the same memory address. Therefore, no additional memory - apart from the memory of the list itself - is needed. As an empty list takes 48 Byte in memory (check via `obj_size(list())`) and 8 Byte for each additional element, our overall prediction for `y` is 4000048 B + 64 B = 4000112 B:
   ...
   Our prediction for `obj_size(x, y)` will end up in the same result, since `y` contains references to the memory address of `x`. Therefore, our prediction is the maximum memory of both objects (y; 4000112 Bytes):
   
    ```{r}
    obj_size(x, y)
    ```
    
   In the next part `y` gets modified and several things happen. When the first element of `y` gets a new value assigned, the element becomes a completely new object. Therefore, it also gets a new address in memory (different to the address of `x`). As 10 is of type double, a silent coercion from integer to double gets triggerd. The calculation for the object size of a double can be figured out in the same way as for integers before. Again, 48 bytes are needed for the empty double and 8 bytes are added per element. Overall 8000040 bytes are now needed for `y`'s first element. As `y`'s elements share no memory anymore, our memory prediction becomes the sum of `y`'s element's object sizes: first element of `y` (8000048 B) + second element of `y` (x; 4000048 B) + list of length 2 (48 B + 16 B) = 12000160 B ~ 12 MB:
   ...
   As `x` is still the second element of `y`, R's memory usage for `x` and `y` together remains the object size of `y` (12000160 B): 
   
    ```{r}
    obj_size(x, y)
    ```
    
   By modifying the second element of `y` in the same way as `y`'s  first element before, the second element will also take 8000048 bytes. As both elements will continue pointing to different memory addresses, `y`'s object size is now 2 * 8000048 B + 64 B = 16000160 B:
   ...
   As also `x` and `y` share no references anymore, their memory usage in R adds up to 20000208 bytes:
