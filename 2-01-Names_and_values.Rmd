...
3. __<span style="color:red">Q</span>__: Predict the output of the following code:
   ...
   __<span style="color:green">A</span>__: To predict the object size of `x` let's first find out how this much memory an empty double occupies and how the size grows with the length of the vector.
   ...
   From the first line of the code chunk above we know that R needs 48 bytes (abbreviated in calculations as B) for an empty double From the second line we can see that R adds 8 bytes per additional element. Don't get confused by the fact that are sometimes (for small vectors) preallocates more than 8 bytes per additional element. This is mainly a practical consideration for better performance. To explain this a bit more in depth, we quote Hadley's explanation from the first edition of Advanced R:
   ...
   However, for our prediction ob `obj_size(x)` we just need to calculate 48 B + 1000000 * 8 B = 8000048 B ~ 8 MB.
   ...
   To predict the size of `y <- list(x, x)` we must pay attention that both list elements of `y` contain references to the same memory address. Therefore, no additional memory apart from the first element - and the memory of the list itself - is needed. An empty list takes 48 Byte in memory (check via `obj_size(list())`) and 8 Byte for each additional element. This gives us a prediction of 8000048 B + 64 B = 8000112 B:
   ...
   As `y` contains references to `x` already, no extra memory is needed for `x`. So our prediction `obj_size(x, y)` will be again 8000112 Bytes:
   
    ```{r}
    obj_size(x, y)
    ```
    
   In the next part the first element of `y` gets modified. As the second element of `y` and `x` point to the same memory address as the first element of `y`, R creates a copy of the first element under a new memory address before it applies the modifications. The newly created first element will still be a double of the same length as before. Therefore, it also needs 8000040 bytes. However, as `y`'s elements don't share any references anymore, our memory prediction adds up to the sum of `y`'s element's object sizes and the length-2 list size: 8000048 B + 8000048 B + 64 B = 16000160 B ~ 16 MB.
   ...
   As the second element of `y` still points to the same address as `x` does, R's memory usage for `x` and `y` together remains to `y`'s object size (16000160 B):
   
    ```{r}
    obj_size(x, y)
    ```
    
   By modifying the second element of `y`, this element now also points to a new memory address. Again the size of the element remains the same. Therefore `y`'s object size remains 16000160 byte:
   ...
   However, as `y` now doesn't share any references with `x` anymore, their memory usage in R now adds up to 24000208 bytes:
