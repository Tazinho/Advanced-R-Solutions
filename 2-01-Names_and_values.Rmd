...
3. __<span style="color:red">Q</span>__: Predict the output of the following code:
   ...
   However, for our prediction of `obj_size(x)` we just need to calculate 48 B + 1000000 * 8 B = 8000048 B ~ 8 MB.
   ...
   To predict the size of `y <- list(x, x)` we must pay attention that both list elements of `y` contain references to the same memory address. Therefore, no additional memory apart from the first element - and the memory of the list itself - is needed. The extra memory for a list of length 2 is 64 bytes (48 byte for an empty list and 8 byte for each element; check for example `obj_size(vector("list", 2))`). This gives us a prediction of 8000048 B + 64 B = 8000112 B:
