...
3. __<span style="color:red">Q</span>__: Predict the output of the following code:
   ...
   __<span style="color:orange">A</span>__: To predict the object sizes let's first find out how much memory an empty integer occupies and how much the memory size grows with the length of the vector.
   ...
   From the first line of the code chunk above we know that R needs 48 Byte for an empty integer. From the second line we can see that R adds on average 4 Byte per additional element. To explain the small irregularities a bit more in depth, we quote Hadley's explanation from the first edition of Advanced R:
   ...
   Now the first part is straight forward as we just need to calculate 48 Byte + 1000000 * 4 Byte = 4000048 ~ 4 MB for our prediction.
   ...
   To predict the size of `y <- list(x, x)` consider that both list elements of `y` point to the same memory address. Therefore no additional memory is needed besides apart from the memory of the list itself. As an empty list takes also 48 Byte in memory and 8 Byte for each additional element our overall prediction for `y` will be 4000048 Byte + 64 Byte = 4000112:
   ...
   Our prediction for `obj_size(x, y)` will end up in the same result. Since `x` and `y` are names with bindings to objects that point to the same reference, no additional memory is needed and our prediction is the maximum memory of both objects (y; 4000112 Bytes):
   
    ```{r}
    obj_size(x, y)
    ```
    
   The next part gets a bit more tricky, as `y` is modified and several things happen. Since the first element of `y` gets a new value assigned, the element becomes a completely new object with a different address in memory. Hence 10 is of type double a silent coercion from integer to double gets triggerd. As an empty double needs 48 Bytes and 8 Bytes per additional element, overall 8000040 Bytes are now needed for `y`'s first element. As `y`'s elements share no memory anymore, the overall prediction can be calculated as the sum of it's elements: first element of `y` (8000048 B) + second element of `y` (x; 4000048 B) + list of length 2 (48 B + 16 B) = 12000160 B ~ 12 MB:
   ...
   As `x` is still an element of `y`, R's memory usage for both of them remains the object size of `y`: 
   
    ```{r}
    obj_size(x, y)
    ```
    
   As in the following line also the second element of `y` is modified in the same way as its first element before, it will now also take 8000048 Byte. However, as both elements have different memory addresses, `y`'s object size is now 2 * 8000048 B + 64 B = 16000160 B:
    ...
    ```
     ...
     As `x` and `y` share no elements anymore, their memory usage in R adds up to 20000208 Bytes:
