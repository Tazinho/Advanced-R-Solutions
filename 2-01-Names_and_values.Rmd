...
1. __<span style="color:red">Q</span>__: Explain why the following code doesn't create a circular list.
   ...
   __<span style="color:orange">A</span>__: The code doesn't create a circular list, since during the creation of the first element copy-on-modify gets triggered. Therefore, the memmory address of `x` changes and the newly created first element of `x` will contain a reference to the old address of `x`. Let us illustrate the details on the code from above:
   
    ```{r}
    # We create x again, determine its address and track any copies that may occur via tracemem.
    x <- list()
    lobstr::obj_addr(x)
    
    # Now we create the first element of x. This should trigger one copy.
    x[[1]] <- x
    
    # Therfore, the memory address and the value of the first element correspond to the initial ones of x.
    lobstr::obj_addr(x[[1]])
    
    # However, x and it's memory address have clearly changed.
    lobstr::obj_addr(x)
    ```
