...
__[A]{.solved}__: We can look up all functions supported by `plotmath` via `?plotmath`. Here we chose to implement a representative subset of `plotmath` functionality within the `to_math()`-DSL. If we wanted to implement more, we would need to be aware of different conversion patterns for symbols and functions and insert the correct conversion in the workflow presented below. Most relevant LaTeX code can be looked up on [Wikipedia](https://en.wikipedia.org/wiki/Help:Displaying_a_formula).
...
Thus, we start building up `latex_env()`, which contains a chain of all the necessary environments which `to_math()` checks to evaluate the expression in.
