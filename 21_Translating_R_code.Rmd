...
1. __[Q]{.Q}__: The escaping rules for `<script>` tags are different because they contain JavaScript, not HTML. Instead of escaping angle brackets or ampersands, you need to escape `</script>` so that the tag isn't closed too early. For example, `script("'</script>'")`, shouldn't generate this:
   ...
   __[A]{.solved}__: To solve this exercise we first recreate the required functions from the book chapter. We then confirm that our code reliably escapes for tags like `<p>` and `<b>`, but doesn't escape correctly for the `<script>` tag. Next, we modify the `escape()` function and redefine the `<script>` tag. Finally we confirm that all of these tags now escape correctly.
   
   Note that `<style>` tags encapsulate css-styling guidelines and the escaping follows the same rules as the `<script>` tag does. We therefore add the same behaviour also for `<style>` tags.
   ...
   Now, we confirm correct escaping for the `<p>` and `<b>` tag functions as well as incorrect behaviour for the `<script>` tag function:
   ...
   Therefore, we modify the `escape()` and the `tag()` function.
   ...
   We create new `<p>` and `<b>` and `<script>` tag functions, which all pass their escaping tests now.
