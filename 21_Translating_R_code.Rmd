...
__[A]{.solved}__: We can look up all functions supported by `plotmath` via `?plotmath`. Here we chose to implement a representative subset of `plotmath` functionality within the `to_math()`-DSL. If we wanted to implement more, we have to observe different conversion patterns for symbols and functions and insert the correct conversion in the workflow presented below. Most relevant LaTeX code can be looked up on  [Wikipedia](https://en.wikipedia.org/wiki/Help:Displaying_a_formula).

We will add support for the following functions:
...
Thus, we start building up `latex_env()`, which contains a chain of all the necessary environments which `to_math()` checks to evaluate our expression in.
...
In this way, `to_math()` will first convert all known Greek letters (found in `greek_env`) and then any other symbols. In this case these symbols are left as is, but converted to the `advr_latex` class.
...
To complete this answer, we will also add the support for unknown functions. Similarly as for the unknown symbols, we use the trick to recursively run the AST and just reuse the code from the textbook.
...
<!-- Do we need to show, that unknown functions are now supported? -->