...
__[Q4]{.Q}__: What is special about the AST below? (Hint: re-read Section 6.2.1)
...
__[Q6]{.Q}__: Construct the expression `if(x > 1) "a" else "b"` using multiple calls to `call2()`. How does the structure code reflect the structure of the AST?
...
In [R 4.0.0](https://cran.r-project.org/doc/manuals/r-release/NEWS.html) `pairwise.t.test()` was updated to use the newly introduces `deparse1()`, which serves as a wrapper around `deparse()`.
