...
4. __[Q]{.Q}__: How much time does `ll_poisson2()` save compared to `ll_poisson1()`? Use `bench::mark()` to see how much faster the optimisation occurs. How does changing the length of `x` change the results?
   ...
   As the redundant calculations within `ll_poisson1()` become more expensive with growing length of `x1`, we expect even further relative performance improvements for `ll_poisson2()`. The following benchmark reveals a relative performance improvement of factor 20 for `ll_poisson2()` when `x1` is of length 1000:
    ...
    bench_df %>% 
      ...
      geom_line() +
      ggtitle(
        "Relative performance of ll_poisson2() over ll_poision1() \nincreases with length of x"
      ) +
      scale_x_log10()
    ```
