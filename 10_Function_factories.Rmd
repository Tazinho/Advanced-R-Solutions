...
When we apply these funcitons to new x values, these are mapped to the lines connecting the initial y values (linear case) or to the same y value as for the next smallest initial x value (constant case).

```{r}
x_new <- sort(runif(1000))
...
To change this bahaviour, one can set `rule = 2`. This leads to the result that for values outside of `range(x)` the boundary values of the function are returned.
...
Further, `approxfun()` provides the option to shift the y values for `method = "constant"` between it's left and right value. According to the documentation this indicates a compromise between left- and right-continuous steps.
...
points(x_new, f_con(x_new), col = "black", pch = 16)
```

Finally, the `ties` argument allows to aggregate y values if multiple ones were provided for the same x. E.g. in the following line we use `mean()` to aggregate these y values before they are used for the interpolation `approxfun(x = c(1,1,2), y = 1:3, ties = mean)`.
